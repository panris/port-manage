...
const tools = [
  {
    name: "scan_ports",
    description:
      "扫描并返回当前机器上所有被占用的端口列表，以及对应的进程信息。用于排查端口冲突、了解系统资源占用情况。",
    inputSchema: {},
  },
  {
    name: "get_port",
  ...
  },
  {
    name: "search_ports",
    description:
      "按关键词（端口号、进程名、PID）搜索端口列表。适用于只知道部分信息时的模糊查找。",
    inputSchema: {
      type: "object",
      properties: {
        q: {
...
const server = new Server(
  { name: "port-manage-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
    ...
    switch (name) {
      case "scan_ports": {
        ...
        const rows = data.data
          .map(
            (p) =>
              `[${p.port}/${p.protocol}] ${p.status}  pid=${p.pid}  ${p.processName}${p.processPath ? " (" + p.processPath + ")" : ""}`
          )
          .join("\n");
        return {
          content: [
      ...
      case "search_ports": {
        ...
        const data = await apiGet<{
          ...
          count: number;
          keyword: string;
        }>(`/api/ports/search?q=${encodeURIComponent(q)}`);
      ...
      case "batch_kill": {
        ...
        const data = await res.json() as {
          ...
          successCount: number;
          failCount: number;
          results: Array<{ pid: number; success: boolean; message: string }>;
        };
